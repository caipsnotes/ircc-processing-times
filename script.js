...
// Load weekly JSON files
async function loadWeeklyData() {
  const weeklyDir = 'weekly/';

  const res = await fetch(`${weeklyDir}index.json`);
  if (!res.ok) {
    console.error('Could not fetch weekly/index.json');
    return;
  }

  const files = await res.json(); 
  files.sort(); 
  const last52 = files.slice(-52);

  for (const file of last52) {
    try {
      const r = await fetch(`${weeklyDir}${file}`);
      if (r.ok) {
  ...
  const datasets = types.map((type, idx) => {
    return {
      ...
      data: labels.map(date => {
        const entry = data.find(d => d.date === date && d.type === type);
        return entry ? entry.time : null;
      }),
      borderColor: idx === 0 ? 'blue' : 'green',
      fill: false
    };
  });
